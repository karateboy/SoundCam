...
object SoundCamInfoHandler {
  ...
  def receive(ac: AcousticImage) = {
    ...
    for(sink <- videoSink) {
      ...
      val videoFrame = new Mat(videoV, videoH, CvType.CV_32FC1)

      Imgproc.resize(frame, videoFrame, new Size(videoH, videoV))
      val ret = Core.minMaxLoc(videoFrame)
      //logger.info(s"max=${ret.maxVal} min=${ret.minVal}")
      val dest = new Mat()

      videoFrame.convertTo(dest, CvType.CV_8UC3)
